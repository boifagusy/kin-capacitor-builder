...
// buildsDir returns the absolute path where built APKs are stored.
func buildsDir() string {
	base := os.Getenv("KIN_DATA_DIR")
	if base == "" {
		if home, err := os.UserHomeDir(); err == nil {
			base = filepath.Join(home, ".local-apk-builder")
		} else {
			base = "."
		}
	}
	return filepath.Join(base, "builds")
}
				...
				if result.Status == StatusSuccess {
					...
					if gh, ok := m.provider.(*GitHubActionsProvider); ok {
						...
						if err := os.MkdirAll(apkDir, 0755); err != nil {
							build.Status = StatusFailed
							build.ErrorMessage = "failed to create builds dir: " + err.Error()
							build.CompletedAt = timePtr(time.Now())
							database.UpdateBuild(build)
							return
						}
