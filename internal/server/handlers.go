...
func (h *Handler) Screen2PostHandler(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("HX-Redirect", "/wizard/3?project_id="+strconv.FormatInt(projectID, 10))
	w.WriteHeader(http.StatusOK)
}
...
// WelcomeHandler renders the PAT setup page
func (h *Handler) WelcomeHandler(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	if err := h.templates.ExecuteTemplate(w, "welcome", nil); err != nil {
		http.Error(w, "Failed to render welcome page", http.StatusInternalServerError)
	}
}
